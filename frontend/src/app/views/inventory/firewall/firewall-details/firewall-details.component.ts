...
export class FirewallDetailsComponent implements OnInit {
  ...
  fetchFirewallData(): void {
    this.firewallService.fetchFirewallData().subscribe((data: Firewall[]) => {
      ...
      if (!control.value || control.value.trim() === "") {
        ...
        return timer(500).pipe(
          switchMap(() => {
            ...
            return firewallService.firewallExists(formattedValue).pipe(
              map((res: any) => {
                if (res.exists) {
  ...
  updateEntry(updatedEntry: Firewall): void {
    if (this.firewallForm.valid && this.uuid) {
      this.firewallService.updateFirewall(updatedEntry, this.uuid).subscribe({
        next: () => {
