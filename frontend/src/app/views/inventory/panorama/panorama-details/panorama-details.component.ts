...
export class PanoramaDetailsComponent implements OnInit {
  ...
  fetchPanoramaData(): void {
    this.panoramaService.panoramaInventory().subscribe((data: Panorama[]) => {
      ...
      if (!control.value || control.value.trim() === "") {
        ...
        return timer(500).pipe(
          switchMap(() => {
            ...
            return panoramaService.panoramaExists(formattedValue).pipe(
              map((res: any) => {
                if (res.exists) {
  ...
  updateEntry(updatedEntry: Panorama): void {
    if (this.panoramaForm.valid && this.uuid) {
      this.panoramaService.updatePanorama(updatedEntry, this.uuid).subscribe({
        next: () => {
